...
main <- function(argv) {
  data <- read.table("../Data/Physical.txt", header = TRUE)
  print(head(data))
  print(skimr::skim(data))

  reg <- lm(Height ~ LeftArm + LeftFoot + HeadCirc + nose, data = data)
  print(reg)
  print(summary(reg))
  fullanova <- anova(reg)
  print(fullanova)
  ...
  reducedreg <- lm(Height ~ LeftArm + LeftFoot, data = data)
  print(reducedreg)
  ...
  leftarmreg <- lm(Height ~ LeftArm, data = data)
  print(leftarmreg)
