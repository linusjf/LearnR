...
main <- function(argv) {
  data <- read.table(handsheight(), header = TRUE)
  print(head(data))
...
plot_handheight <- function(data, reg) {
  ...
  main_label <- paste("HandSpan versus Height\n", coefs["(Intercept)"], "+", coefs["HandSpan"], 
    "HandSpan")
  plot(data$HandSpan, data$Height, main = main_label, xlab = "HandSpan", ylab = "Height", 
    pch = 19, frame = FALSE)
  abline(reg, col = "blue")
