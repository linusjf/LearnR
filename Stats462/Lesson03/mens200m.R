...
mens200m() <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/mens200m.txt")
}
library(skimr)

main <- function(argv) {
  data <- read.table(mens200m(), header = TRUE)
  print(head(data))
...
plot_mens200m <- function(data, reg) {
  ...
  main_label <- paste("Year versus Men's 200 m timings\n", coefs["(Intercept)"], 
    coefs["Year"], "Year")
  plot(data$Year, data$Men200m, main = main_label, xlab = "Year", ylab = "Men's 200m timings", 
    pch = 19, frame = FALSE)
  abline(reg, col = "blue")
