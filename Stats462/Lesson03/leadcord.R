...
leadcord() <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/leadcord.txt")
}
library(skimr)

main <- function(argv) {
  data <- read.table(leadcord(), header = TRUE)
  print(head(data))
...
plot_leadcord <- function(data, reg) {
  ...
  main_label <- paste("monthly gasoline
                       lead sales (in metric tons) versus mean lead
    concentrations (µl/dl) in umbilical cord blood of babies\n", 
    coefs["(Intercept)"], "+", coefs["Sold"], "Sold")
  plot(data$Sold, data$Cord, main = main_label, xlab = "Sold", ylab = "Cord", pch = 19, 
    frame = FALSE)
  abline(reg, col = "blue")
