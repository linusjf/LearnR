#!/usr/bin/env Rscript
anscombe.txt <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/anscombe.txt")
}
library(skimr)
...
plot_stdresid_predictor <- function(data, reg) {
  ...
  plot(data$x3, std_residuals, main = main_label, xlab = "x3", ylab = "Standardized Residuals", 
    pch = 19, frame = TRUE)
...
plot_predictor <- function(data, reg) {
  ...
  plot(data$x3, residuals, main = main_label, xlab = "x3", ylab = "Residuals", 
    pch = 19, frame = TRUE)
  abline(h = mean(residuals), col = "black", lty = "dashed")
}
...
plot_anscombe <- function(data, reg) {
  ...
  main_label <- paste("x3 versus y3\n", coefs["(Intercept)"], x <- if (sign(coefs["x3"]) == 
    1) {
    "+"
  } else {
    "-"
  }, abs(coefs["x3"]), "x3")
  plot(data$x3, data$y3, main = main_label, xlab = "x3", ylab = "y3", pch = 19, 
    frame = TRUE)
  abline(reg, col = "blue")
  summ <- summary(reg)
