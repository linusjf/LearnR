...
plot_stdresid_predictor <- function(data, reg) {
  ...
  plot(data$Speed, std_residuals,
    main = main_label, xlab = "Speed", ylab = "Standardized Residuals",
    pch = 19, frame = TRUE
  )
...
plot_predictor <- function(data, reg) {
  ...
  plot(data$Speed, residuals,
    main = main_label, xlab = "Speed", ylab = "Residuals",
    pch = 19, frame = TRUE
  )
  abline(h = mean(residuals), col = "black", lty = "dashed")
}
...
plot_sqrtcarstopping <- function(data, reg) {
  ...
  plot(data$Speed, sqrt(data$StopDist),
    main = main_label, xlab = "Speed", ylab = "Sqrt(StopDist)",
    pch = 19, frame = TRUE
  )
  abline(reg, col = "blue")
  summ <- summary(reg)
...
plot_carstopping <- function(data, reg) {
  ...
  plot(data$Speed, data$StopDist,
    main = main_label, xlab = "Speed", ylab = "StopDist",
    pch = 19, frame = TRUE
  )
  abline(reg, col = "blue")
  summ <- summary(reg)
