...
alcoholtobacco.txt <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/alcoholtobacco.txt")
}
library(skimr)

main <- function(argv) {
  data <- read.table(alcoholtobacco.txt(), header = TRUE)
  print(head(data))
...
plot_stdresid_predictor <- function(data, reg) {
  ...
  plot(data$Tobacco, std_residuals, main = main_label, xlab = "Tobacco", ylab = "Standardized Residuals", 
    pch = 19, frame = TRUE)
...
plot_predictor <- function(data, reg) {
  ...
  plot(data$Tobacco, residuals, main = main_label, xlab = "Tobacco", ylab = "Residuals", 
    pch = 19, frame = TRUE)
  abline(h = mean(residuals), col = "black", lty = "dashed")
}
...
plot_alcoholtobacco <- function(data, reg) {
  ...
  main_label <- paste("Alcohol versus Tobacco\n", coefs["(Intercept)"], x <- if (sign(coefs["Tobacco"]) == 
    1) {
    "+"
  } else {
    "-"
  }, abs(coefs["Tobacco"]), "Tobacco")
  plot(data$Tobacco, data$Alcohol, main = main_label, xlab = "Tobacco", ylab = "Alcohol", 
    pch = 19, frame = TRUE)
  abline(reg, col = "blue")
  summ <- summary(reg)
