...
process_infection_risk <- function(data) {
  ...
  ci <- predict(reg, se.fit = TRUE, newdata, interval = "confidence")
  print(ci)
  print("Prediction Interval")
  predi <- predict(reg, se.fit = TRUE, newdata, interval = "prediction")
  print(predi)
...
plot_stdresid_predictor <- function(data, reg) {
  ...
  plot(data$Stay, std_residuals, main = main_label, xlab = "Stay", ylab = "Standardized Residuals", 
    pch = 19, frame = TRUE)
...
plot_predictor <- function(data, reg) {
  ...
  plot(data$Stay, residuals, main = main_label, xlab = "Stay", ylab = "Residuals", 
    pch = 19, frame = TRUE)
  abline(h = mean(residuals), col = "black", lty = "dashed")
}
...
plot_infection_risk <- function(data, reg, ci, predi) {
  ...
  main_label <- paste("Stay versus Infection Risk\n", coefs["(Intercept)"], x <- if (sign(coefs["Stay"]) == 
    1) {
    "+"
  } else {
    "-"
  }, abs(coefs["Stay"]), "Stay")
  plot(data$Stay, data$InfctRsk, main = main_label, xlab = "Stay", ylab = "Infection Risk", 
    pch = 19, frame = TRUE)
  abline(reg, col = "blue")
  matlines(data$Stay, ci[, c("lwr", "upr")], col = "red", lty = 1, type = "l")
  matlines(data$Stay, predi[, c("lwr", "upr")], col = "blue", lty = 1, type = "l")
  polygon(c(data$Stay, rev(data$Stay)), c(predi[, "upr"], rev(predi[, "lwr"])), 
    col = adjustcolor("orangered", alpha.f = 0.7), border = NA)
  polygon(c(data$Stay, rev(data$Stay)), c(ci[, "upr"], rev(ci[, "lwr"])), col = adjustcolor("wheat", 
