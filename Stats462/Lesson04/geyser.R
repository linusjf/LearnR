...
oldfaithfulgeyser.txt <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/Old_Faithful_geyser.txt")
}
library(skimr)

main <- function(argv) {
  data <- read.table(oldfaithfulgeyser.txt(), header = TRUE)
  print(head(data))
...
plot_stdresid_predictor <- function(data, reg) {
  ...
  plot(data$eruption, std_residuals, main = main_label, xlab = "Eruption", ylab = "Standardized Residuals", 
    pch = 19, frame = TRUE)
...
plot_predictor <- function(data, reg) {
  ...
  plot(data$eruption, residuals, main = main_label, xlab = "Eruption", ylab = "Residuals", 
    pch = 19, frame = TRUE)
  abline(h = mean(residuals), col = "black", lty = "dashed")
}
...
plot_geyser <- function(data, reg) {
  ...
  main_label <- paste("Eruption versus Waiting\n", coefs["(Intercept)"], x <- if (sign(coefs["eruption"]) == 
    1) {
    "+"
  } else {
    "-"
  }, abs(coefs["eruption"]), "eruption")
  plot(data$eruption, data$waiting, main = main_label, xlab = "Eruption", ylab = "Waiting", 
    pch = 19, frame = TRUE)
  abline(reg, col = "blue")
  summ <- summary(reg)
