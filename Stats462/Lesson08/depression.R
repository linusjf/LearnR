...
depression.txt <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/depression.txt")
}
...
main <- function(argv) {
  data <- read.table(depression.txt(), header = TRUE)
  print(head(data))
...
plot_fitted_lines <- function(data) {
  ...
  suppressWarnings(abline(lm, col = "black", lty = 2))
  ...
  eqn <- model_equation(lm, digits = 4, trim = TRUE)
  suppressWarnings(abline(lm, col = "red", lty = 4))
  slope <- lm$coefficients["age"]
  angle <- abs(rad2deg(atan(slope)))
