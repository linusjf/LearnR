...
main <- function(argv) {
  ...
  newdata <- data.frame(Gest = c(38, 38),
                        Smoke = c(0, 1))
  ...
  summary <-
    data.frame(
               `Model estimated using…` =
                 c("all 32 data points"),
               `SE(Gest)` = c(segest),
               `Width of CI for μY` =
                 paste(toString(names(width)),
                                   toString(width))
               )
  ...
  fit <- prediction$fit
  width <- fit[, 3] - fit [, 2]
  summ0 <- data.frame(`Model estimated using…` = c("16 non-smokers"),
                        `SE(Gest)` = c(segest),
  ...
  fit <- prediction$fit
  width <- fit[, 3] - fit [, 2]
  summ1 <- data.frame(`Model estimated using…` = c("16 smokers"),
                        `SE(Gest)` = c(segest),
                        `Width of CI for μY` = c(paste(width, collapse = " "))
                        )
