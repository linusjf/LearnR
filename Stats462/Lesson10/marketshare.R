...
main <- function(argv) {
  ...
  discounts <- data %>%
    filter(Discount == 1)
  with(no_discounts, plot(fitted, residuals,
    col = "blue", pch = 15, main = "OLS Scatterplot of residuals versus fits",
  ...
  discounts <- data %>%
    filter(Discount == 1)
  with(no_discounts, plot(fitted.wls, residuals.wls,
    col = "blue", pch = 15, main = "WLS Scatterplot of standardized residuals versus fits",
    xlab = "Fitted", ylab = "Standardized Residuals", xlim = c(
      min(data$fitted.wls),
      max(data$fitted.wls)
