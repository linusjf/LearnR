...
main <- function(argv) {
  ...
  plot(
    x = num.responses, y = cost, ylim = c(min(cost), max(cost)), panel.last = c(lines(sort(num.responses),
      fitted(model.1)[order(num.responses)],
      col = "blue"
    ), lines(sort(num.responses),
      fitted(model.2)[order(num.responses)],
      col = "red"
    )), col.main = "blue",
