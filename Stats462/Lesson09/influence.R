...
noinfluence <- function(argv) {
  data <- read.table(influence1.txt(), header = TRUE)
  print(head(data))
...
outlieronly <- function(argv) {
  data <- read.table(influence2.txt(), header = TRUE)
  print(head(data))
  print(skimr::skim(data))
  plot(data$x, data$y, xlab = "X", ylab = "Y", main = "Scatter plot of y versus x")
  data1 <- head(data, -1)
  ...
  labels <- c(eqn, eqn2)
  cols <- c("black", "red")
  legend("bottomright", col = cols, legend = labels, lty = 1:2, text.col = cols)
}

leverage <- function(argv) {
  data <- read.table(influence3.txt(), header = TRUE)
  print(head(data))
  print(skimr::skim(data))
  plot(data$x, data$y, xlab = "X", ylab = "Y", main = "Scatter plot of y versus x")
  data1 <- head(data, -1)
...
all <- function(argv) {
  data <- read.table(influence4.txt(), header = TRUE)
  print(head(data))
  print(skimr::skim(data))
  plot(data$x, data$y, xlab = "X", ylab = "Y", main = "Scatter plot of y versus x")
  data1 <- head(data, -1)
