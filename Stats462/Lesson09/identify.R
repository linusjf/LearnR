...
plot_fdistr <- function(data, model, path) {
  ...
  plot(x, df(x = x, df1 = df1, df2 = df2), main = paste0("F distribution (", df1, 
    ",", df2, ")"), sub = path, type = "l", xlim = c(0, max(data$x)), ylab = "Probability Density Function")
...
plot_dffits <- function(data, model, path) {
  ...
  plot(data$x, data$dffits, xlab = "X", ylab = "DFFITS", main = paste0("Standardized dffits\n", 
    path), ylim = c(miny, maxy), sub = paste0("Critical value = (+/-)", round(cv, 
    4)))
