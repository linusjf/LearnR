...
shortleaf.txt <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/shortleaf.txt")
}
...
main <- function(argv) {
  data <- read.table(shortleaf.txt(), header = TRUE)
  print(head(data))
  ...
  # What is the 'average' volume of all shortleaf pine trees that are 10' in
  # diameter?
  lndiam10 <- log(10)
  newdata <- data.frame(lndiam = lndiam10)
  prediction <- predict(reg, newdata, se.fit = TRUE, interval = "confidence")
  fit <- prediction$fit
  fit <- exp(fit)
  print(fit)
