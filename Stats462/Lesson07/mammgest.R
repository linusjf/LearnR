...
mammgest.txt <- function() {
  library(rprojroot)
  paste0(find_root(has_file(".Rprofile")), "/Stats462/Data/mammgest.txt")
}
...
main <- function(argv) {
  data <- read.table(mammgest.txt(), header = TRUE)
  print(head(data))
  ...
  eqn <- paste(round(coefs["(Intercept)"], 4), sprintf("%+0.4f", round(coefs["Birthwgt"], 
    4)), "Birthwgt")
  with(data, plot(Birthwgt, Gestation, main = "Scatterplot: Birthweight versus Gestation", 
    sub = eqn))
  ...
  with(data, {
    abline(reg, col = "red")
    lines(lowess(Birthwgt, Gestation), col = "blue")
  ...
  eqn <- paste(round(coefs["(Intercept)"], 4), sprintf("%+0.4f", round(coefs["Birthwgt"], 
    4)), "Birthwgt")
  with(data, plot(Birthwgt, lngestation, main = "Scatterplot: Birthweight versus log(Gestation)", 
    sub = eqn))
  labels <- c(paste0("Sigma: ", round(summ$sigma, 4)), paste0("R-squared: ", round(summ$r.squared, 
    4)), paste0("Adj R-squared: ", round(summ$adj.r.squared, 4)))
  with(data, {
    abline(reg, col = "red")
    lines(lowess(Birthwgt, lngestation), col = "blue")
    legend("topright", legend = labels)
  })
  plot(reg, which = 1, caption = "Residuals versus Fitted")
  ...
  prediction <- predict(reg, newdata, se.fit = TRUE, interval = "prediction")
