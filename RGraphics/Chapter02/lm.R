...
main <- function(argv) {
  ...
  plot(lm_sr <- lm(sr ~ pop15 + pop75 + dpi + ddpi, data = LifeCycleSavings), id.n = 1, 
    cex.caption = 0.8, which = 1:6, panel = function(...) {
  ...
  plot(predict(lm_sr), LifeCycleSavings$sr, xlab = "predicted", ylab = "actual")
  abline(a = 0, b = 1)
  return(0)
}
