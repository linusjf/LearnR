...
nvessscale <- function(params, xlim, ylim, new = TRUE) {
  ...
  plot(0:1, c(1, params$N), type = "n", axes = !new, ann = FALSE, xlim = xlim, 
    ylim = ylim)
}
...
compscale <- function(params, xlim, ylim, new = TRUE) {
  ...
  plot(0:1, c(1, params$T), type = "n", axes = !new, ann = FALSE, xlim = xlim, 
    ylim = ylim)
}
...
crit_p <- function(comp, params) {
  c(uniroot(lower_ci, c(1e-05, 1), n = params$N, breaks = 1/params$T, lb = max(comp))$root, 
...
anncomp <- function(params, comp, xlim, ylim, cylim) {
  ...
  polygon(c(cp[2], cp[2], 0, 0, cp[1], cp[1]), c(0, nv[2], nv[2], nv[1], nv[1], 
...
plot_pars <- function(params, comp, xlim = NULL, ylim = NULL) {
  ...
  mtext(paste("N = ", round(params$N), "     brokenness = ", round(1/params$T, 
    3), sep = ""), side = 3, line = 2)
}
