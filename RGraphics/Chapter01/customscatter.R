...
est_n <- function(comp, broke, n) {
  t <- 1 / broke
  n / (1 - (1 - uniroot(ecomp, c(0.00001, 1),
    t = t,
    comp = comp
  )$root)^t)
}
...
crit_p <- function(comp, params) {
  c(
    uniroot(lower_ci, c(0.00001, 1),
      n = params$N,
      breaks = 1 / params$T, lb = max(comp)
    )$root,
    if (upper_ci(0.00001, params$N, 1 / params$T, min(comp)) > 0) {
      0
    } else {
      uniroot(upper_ci, c(0.00001, 1),
        n = params$N,
        breaks = 1 / params$T, lb = min(comp)
      )$root
    }
  )
}
