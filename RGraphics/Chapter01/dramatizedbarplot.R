...
main <- function(argv) {
  ...
  # nolint start
  pushViewport(plotViewport(c(3, 2, 2, 1)), viewport(xscale = c(1991, 2003), yscale = c(
    0,
    250
  )))
  grid::grid.rect()
  ...
  # black bars
  grid::grid.rect(x = unit(year, "native"), y = 0, width = unit(1, "native"), height = unit(
    maxpop,
    "native"
  ), just = "bottom", gp = grid::gpar(fill = "black"))
  # tiger in bars
  tiger_grob <- pictureGrob(tiger, x = 0.45, FUN = grobify)
  # Start from 2 because bar 1 does not overlap with tiger
  for (i in 2:length(year)) {
    grid::grid.clip(
      x = unit(year[i], "native"), y = 0, width = unit(1, "native"),
