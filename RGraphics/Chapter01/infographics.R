...
main <- function(argv) {
  ...
  grid::grid.polygon(x, y, gp = grid::gpar(col = NA, fill = rgb(
    0.67, 0, 0.11,
    0.7
  )))
  label1 <- textGrob("UNACCOUNTED\nFOR", unit(0.2, "npc") - unit(2, "mm"), unit(
    0.6,
    "npc"
  ) + unit(2, "mm"), gp = grid::gpar(cex = 1.4, fontface = "bold"), just = c(
    "right",
    "bottom"
  ))
  grid.rect(0.2, 0.6, just = c("right", "bottom"), width = grobWidth(label1) +
    unit(4, "mm"), height = grobHeight(label1) + unit(4, "mm"), gp = grid::gpar(
    col = NA,
    fill = rgb(1, 1, 1, 0.5)
  ))
  grid::grid.draw(label1)
  label2 <- textGrob("ACCOUNTED\nFOR", unit(0.8, "npc") + unit(2, "mm"), unit(
    0.6,
    "npc"
  ) + unit(2, "mm"), gp = grid::gpar(cex = 1.4, fontface = "bold"), just = c(
    "left",
  ...
  grid.rect(0.8, 0.6, just = c("left", "bottom"), width = grobWidth(label2) + unit(
    4,
    "mm"
  ), height = grobHeight(label2) + unit(4, "mm"), gp = gpar(col = NA, fill = rgb(
    1,
    1, 1, 0.5
  )))
  grid::grid.draw(label2)
  grid::grid.segments(c(0.2, 0.8), 0.6, c(0.3, 0.7), 0.5, gp = gpar(
    lty = "dotted",
    lwd = 2
  ))
...
$8.7 billion of the $9.1 billion it spent on reconstruction in Iraq",
    x = unit(0.5, "cm"), y = unit(3, "lines"), just = c("left", "top"), gp = gpar(
      cex = 1,
      col = "white"
    )
  )
  ...
  grid.segments(x0 = unit(0.5, "cm"), x1 = unit(1, "npc") - unit(0.5, "cm"), y0 = unit(
    1,
    "npc"
  ) - unit(2, "lines"), y1 = unit(1, "npc") - unit(2, "lines"), gp = grid::gpar(
    col = "grey50",
    lwd = 2
  ))
  grid.text("That's 96 Percent", x = unit(0.5, "cm"), y = unit(1, "npc") - unit(
    1,
    "lines"
  ), just = "left", gp = grid::gpar(fontface = "bold", col = "white"))
