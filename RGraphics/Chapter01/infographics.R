...
main <- function(argv) {
  pic <- read.pnm(system.file("extra", "AfterTheBombs.pnm",
    package =
      "RGraphics"
  ))
  ...
  label1 <- textGrob("UNACCOUNTED\nFOR",
    ...
    gp = grid::gpar(cex = 1.4, fontface = "bold"),
    just = c("right", "bottom")
  )
  grid.rect(.2, .6,
    just = c("right", "bottom"),
    ...
    gp = grid::gpar(col = NA, fill = rgb(1, 1, 1, .5))
  )
