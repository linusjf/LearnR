...
main <- function(argv) {
  par(mar = rep(1, 4))
  map("nzHires", fill = TRUE, col = "gray80", regions = c(
  ...
  maplocs <- map(
    projection = "sp_mercator", wrap = TRUE, lwd = 0.1, col = "gray",
  ...
  par(fig = c(0.99 - 0.5, 0.99, 0.01, 0.01 + 0.5 * aspect * 4.5 / 6.5), mar = rep(
    0,
    4
  ), new = TRUE)
  plot.new()
  ...
  map(
    projection = "sp_mercator", wrap = TRUE, lwd = 0.5, ylim = c(-60, 75), interior = FALSE,
