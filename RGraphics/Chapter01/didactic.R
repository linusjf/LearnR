...
# Auckland).  Lots of coordinate transformations being done 'by hand'.  This code
# is not really reusable; just a demonstration that very pretty results are
# possible if you're sufficiently keen.


main <- function(argv) {
  par(mfrow = c(2, 1), pty = "s", mar = rep(1, 4))
  # Create plotting region and plot outer circle
  plot(c(-1.1, 1.2), c(-1.1, 1.2), type = "n", xlab = "", ylab = "", xaxt = "n", 
    yaxt = "n", cex.lab = 2.5)
  ...
  lines(c(0, 0), c(k2 * sin(pi/2), 1.11), lwd = 4)
  lines(c(0, 0), c(-1, -1.12), lwd = 4)
  a2x <- sin(-pi/4)
  a2y <- cos(-pi/4) * (-k1)
  ...
  yy1 <- k2 * sin(theta1s[vals >= 0]) - k1 * cos(theta1s[vals >= 0]) * cos(theta2s[vals >= 
    0] + pi/4)
  theta2s <- -theta2s
  vals <- k1 * sin(theta1s) + k2 * cos(theta1s) * cos(theta2s + pi/4)
  xx3 <- cos(theta1s[vals >= 0]) * sin(theta2s[vals >= 0] + pi/4)
  yy3 <- k2 * sin(theta1s[vals >= 0]) - k1 * cos(theta1s[vals >= 0]) * cos(theta2s[vals >= 
    0] + pi/4)
  ...
  # Plot Latitudes
  vals2 <- c(-1.5, 0, 1.5, 3, 4.5, 5.6) * pi/12
  for (phi in vals2) {
    if (k1 * sin(phi) > k2 * cos(phi)) {
      crit <- pi
    } else {
