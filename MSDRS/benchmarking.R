...
find_records_2 <- function(datafr, threshold) {
  datafr <- datafr %>%
    mutate(
      over_threshold = (temp >= threshold), cummax_temp = (temp == cummax(temp)),
...
main <- function(argv) {
  ...
  record_temp_perf_2 <- microbenchmark(find_records_1(chicagoNMMAPS, 27), find_records_2(
    chicagoNMMAPS,
    27
  ))
