...
find_records_2 <- function(datafr, threshold) {
  datafr <- datafr %>%
    mutate(over_threshold = (temp >= threshold), cummax_temp = (temp == cummax(temp)), 
...
main <- function(argv) {
  print(microbenchmark(a <- rnorm(1000), b <- mean(rnorm(1000))))
  example_data <- tibble(date = c("2015-07-01", "2015-07-02", "2015-07-03", "2015-07-04", 
    "2015-07-05", "2015-07-06", "2015-07-07", "2015-07-08"), temp = c(26.5, 27.2, 
    28, 26.9, 27.5, 25.9, 28, 28.2))
  ...
  record_temp_perf <- microbenchmark(find_records_1(example_data, 27), find_records_2(example_data, 
    27))
  print(record_temp_perf)
  record_temp_perf_2 <- microbenchmark(find_records_1(chicagoNMMAPS, 27), find_records_2(chicagoNMMAPS, 
    27))
  print(record_temp_perf_2)
  microbenchmark::autoplot(record_temp_perf)
  ggplot2::ggsave("firstperftest.pdf")
  microbenchmark::autoplot(record_temp_perf_2)
