...
main <- function(argv) {
  ext_tracks_file <<- paste0("http://rammb.cira.colostate.edu/research/", "tropical_cyclones/tc_extended_best_track_dataset/", 
  ...
  ext_tracks <<- readr::read_fwf(ext_tracks_file, fwf_widths(ext_tracks_widths, 
    ext_tracks_colnames), na = "-99")
  sampling()
  summarizing()
  grouping()
  selecting()
...
selecting <- function() {
  print(ext_tracks %>%
    dplyr::select(storm_name, month, day, hour, year, latitude, longitude, max_wind))
