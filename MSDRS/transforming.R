...
main <- function(argv) {
  ...
  ext_tracks_colnames <- c(
    "storm_id", "storm_name", "month", "day", "hour", "year",
  ...
  print(andrew_tracks %>%
    select(datetime) %>%
    mutate(
      year = year(datetime), month = months(datetime), weekday = weekdays(datetime),
  ...
  check_tracks <- ext_tracks %>%
    ...
    ), month = months(datetime), month = factor(month, levels = c(
      "April",
  ...
  if (library(ggmap, logical.return = TRUE)) {
    ...
    ggmap(miami) + geom_path(
      data = andrew_tracks, aes(x = -longitude, y = latitude),
      color = "gray", size = 1.1
    ) + geom_point(data = andrew_tracks, aes(
      x = -longitude,
      y = latitude, color = date
    ), size = 2)
    ggplot2::ggsave("miamimap.pdf")
  }
  andrew_tracks <- andrew_tracks %>%
    mutate(datetime = with_tz(datetime, tzone = "America/New_York"), date = format(
      datetime,
      "%b %d"
    ))
  print(andrew_tracks)
