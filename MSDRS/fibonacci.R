...
main <- function(argv) {
  ...
  fib_data <- map(1:20, function(x) {
    log(microbenchmark::microbenchmark(fib(x), times = 100)$time)
  })
  ...
  pdf("speed.pdf")
  plot(1:20, fib_data$med_time, xlab = "Fibonacci Number", ylab = "Log Median Time
     (log Nanoseconds)", 
    pch = 18, bty = "n", xaxt = "n")
  axis(1, at = 1:20)
  points(1:20 + 0.1, memo_data$med_time, col = "blue", pch = 18)
  legend(1, max(fib_data$med_time), c("Not Memoized", "Memoized"), pch = 18, col = c("black", 
    "blue"), bty = "n", cex = 1, y.intersp = 1.5)
  gra
ics.off()
  return(0)
}
