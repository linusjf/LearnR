...
main = function(argv) {
    ...
    for (i in 1:length(timepoints)) z[i] <- nsz(14.084, -3.4107, 0.0015, 1.8832, 
        timepoints[i])
    print(z)
    ...
    plot(timepoints, beta0s, xlab = "Time", ylab = expression(paste(beta, "0 component (N/S)")))
    lines(timepoints, beta0s)
    plot(timepoints, beta1s, xlab = "Time", ylab = expression(paste(beta, "1 component (N/S)")))
    lines(timepoints, beta1s)
    plot(timepoints, beta2s, xlab = "Time", ylab = expression(paste(beta, "2 component (N/S)")))
