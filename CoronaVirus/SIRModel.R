...
parms <- list(gamma = 1/14, gamma_lcl = 1/15, gamma_ucl = 1/13, inf = 1, inf_ucl = 1, 
...
analyse <- function(init, infected, dates, death_rate, deaths, popn, plot_data) {
  ...
  with(parms, {
    opt <<- optim(c(inf, act, gamma), rss, method = "L-BFGS-B", lower = c(inf_lcl, 
      act_lcl, gamma_lcl), upper = c(inf_ucl, act_ucl, gamma_ucl), control = list(maxit = length(infected) * 
  ...
  start_date <- dates[1]
  matplot(fit$time, fit[, 2:4], type = "l", xlab = "Date", ylab = "Number of
          subjects", 
    lwd = 2, lty = 1, col = col, xaxt = "n")
  axis(1, at = fit$time[fit$time%%100 == 0], labels = seq(from = start_date, length.out = 365, 
    by = "day")[c(100, 200, 300)])
  title(plot_data$label, outer = TRUE, line = -2)
  legend("bottomright", c("Susceptibles", "Infecteds", "Recovereds"), lty = 1, 
    lwd = 2, col = col, inset = 0.05)

  suppressWarnings(matplot(fit$time, fit[, 2:4], type = "l", xlab = "Date", ylab = "Number of
          subjects", 
  ...
  print(r0)
  names(opt_par) <- c("prob of infection (inf)", "activity rate (act)", "Rate of recovery (gamma)", 
    "Total population")
  print(opt_par)
