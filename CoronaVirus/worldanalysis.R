...
download_csv <- function(filename) {
  ...
  path <- paste0("csse_covid_19_data/csse_covid_19_time_series/", filename)
  ...
  print("")
  must_download <- !file.exists(destination) | mtime < git_date
  if (must_download) {
    url <- d$url
...
clean_data <- function(data) {
  ...
  data %<>%
    gather(key = date, value = count, -country, na.rm = TRUE)
...
plot_world_cases <- function(data, ready_data) {
  ...
  data_long <- data %>%
    select(c(country, date, confirmed, active.cases, recovered, deaths)) %>%
    gather(key = type, value = count, -c(country, date))
  ## set factor levels to show them in a desirable order
  data_long %<>%
    mutate(type = factor(type, c("confirmed", "active.cases", "recovered", "deaths")))
  ## cases by type
  df <- data_long %>%
...
ready_plot_data <- function(data) {
  ...
  top_countries %<>%
    setdiff("Others") %>%
    c("Others")
  ready_data <- list(max.date = max_date, data.latest = data_latest, top.countries = top_countries)
  return(ready_data)
}
