...
main <- function(argv) {
  ...
  ggplot() +
    ...
    geom_point(
      data = datacov, aes(
        x = Long, y = Lat,
        size = eval(parse(text = yday)), color = eval(parse(text = yday))
      ), stroke = F,
      alpha = 0.7
    ) +
    scale_size_continuous(name = "Cases", trans = "log", range = c(
      1,
      7
    ...
    scale_color_viridis_c(
      option = "inferno", name = "Cases", trans = "log", breaks = mybreaks,
    ...
    theme(
      legend.position = "bottom", text = element_text(color = "#22211d"),
      plot.background = element_rect(fill = "#ffffff", color = NA), panel.background = element_rect(
        fill = "#ffffff",
        color = NA
