...
download_csv <- function() {
  ...
  # nolint end
  myopts <- curlOptions(useragent = "LearnR", ssl.verifypeer = FALSE)

  url <- getURL(paste0(repo, "commits?path=", path), useragent = "LearnR", ssl.verifypeer = FALSE)
  d <- rjson::fromJSON(url)[[1]]
  git_date <- as.POSIXct(d$commit$author$date)
  must_download <- !file.exists(destination) | file.info(destination)$mtime < git_date
  if (must_download) {
    url <- d$url
...
main <- function(argv) {
  ...
  download_csv()
  datacov <- readr::read_csv("confirmed.csv", skip_empty_rows = TRUE)
  yday <- Sys.Date() - 1
  ...
  datacov <- tidyr::drop_na(datacov, tidyselect::all_of(ydaystring))
  ## get the world map
  ...
  ggplot() + geom_polygon(data = world, aes(x = long, y = lat, group = group), 
    fill = "grey", alpha = 0.3) + geom_point(data = datacov, aes(x = Long, y = Lat, 
    size = eval(parse(text = yday)), color = eval(parse(text = yday))), stroke = F, 
    alpha = 0.7) + scale_size_continuous(name = "Cases", trans = "log", range = c(1, 
    7), breaks = mybreaks, labels = c("1-19", "20-99", "100-999", "1,000-49,999", 
    "50,000+")) + # scale_alpha_continuous(name='Cases', trans='log', range=c(0.1,
  # 0.9),breaks=mybreaks) +
  scale_color_viridis_c(option = "inferno", name = "Cases", trans = "log", breaks = mybreaks, 
    ...
    guides(colour = guide_legend()) + labs(caption = "Data Repository provided by Johns Hopkins CSSE.
