...
main <- function(argv) {
  ...
  d1c <- d1 %>%
    ...
    summarise(
      `10000` = min(date[cc > 10000]),
      `100000` = min(date[cc > 100000])
    ) %>%
    gather(variable, date) %>%
    left_join(d1, by = "date") %>%
    mutate(label = paste0(format(as.numeric(variable), big.mark = ",",
                                 scientific = FALSE), "\ncases"))
...
surviving cases."
  ...
  d2 %>%
    ...
    geom_line() +
    geom_text(
      data = filter(d2, date == max(date)), aes(label = Country.Region),
  ...
  +
    ...
    labs(
      ...
      subtitle = "Eight countries with most diagnosed cases; Iran's early values
      truncated.\nA high level of uncertainty reflecting rapidly changing
      denominators as well as many unresolved cases."
    ) +
